...
if choice1 == "1":
    for video_url in playlist.video_urls:
        ...
        try:
            YouTube(video_url).streams.get_highest_resolution().download(f"{SAVE_PATH}/{dirname}/MP4")
            print(f"Download {x} of {len(playlist.video_urls)} completed.")
        except:
            print("File could not be downloaded, skipping...")
        try:
            os.rename(f"{SAVE_PATH}/{dirname}/MP4/Youtube.mp4", f"{SAVE_PATH}/{dirname}/MP4/Youtube{random.randint(1000000, 9999999)}.mp4")
            print("")
        except:
            print("")
        x = x+1
